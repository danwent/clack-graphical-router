...
public class Rule
	...
	public void paintComponent(Graphics g) {
		...
		Point2D p = graph.toScreen(new Point(activeoffset, activelength));
		...
		g.setColor(Color.lightGray);
		if (orientation == HORIZONTAL)
			g.fillRect((int)p.getX(), drawHere.y, (int)p.getY(), drawHere.height);
		else
			g.fillRect(drawHere.x, (int)p.getX(), drawHere.width, (int)p.getY());
