...
import net.clackrouter.jgraph.pad.GPBarEntry;
import net.clackrouter.jgraph.pad.GPPlugin;
 ...
 *  and the toolbars seen at the top of a clack window.  
 *	(This class if based on JGraph code) 
 *
 */
public class ClackBarFactory implements TranslatorConstants {
	 ...
	 */
	protected JMenu createClackAddMenu(){
		// add in menu to add components
		ClackAddComponent a = (ClackAddComponent)getAction("ClackAddComponent");
		JMenu addMenu = new JMenu("Add");
		String key = "ClackAddCompMenus";
		String[] menus = tokenize(key, Translator.getString(key));
		
		for(int i = 0; i < menus.length; i++){
			String menuName = menus[i];
			JMenu subMenu = new JMenu(menuName);
			addMenu.add(subMenu);
			String[] menuContents = tokenize(menuName, Translator.getString(menuName + SUFFIX_MENU));
			for(int j = 0; j < menuContents.length; j++){
				JMenuItem item = new JMenuItem(menuContents[j]);
				item.addActionListener(a);
				subMenu.add(item);
			}
		}
		
		String[] additional_comps = a.getMenuNames();
		if(additional_comps.length > 0){
			JMenu additionalMenu = new JMenu("Additional");
			addMenu.add(additionalMenu);
			for(int i = 0; i < additional_comps.length; i++){
				JMenuItem item = new JMenuItem(additional_comps[i]);
				item.addActionListener(a);
				additionalMenu.add(item);			
			}
		}
	 ...
	 * 
	 * @param cell the ComponentCell that this popup is being created for, or null 
	 */
	public JPopupMenu createGraphPopupMenu(ComponentCell cell) {
		String key = GRAPH_POPUP;
		JPopupMenu pop = new JPopupMenu();
		final Object cell_final = cell;
		
		pop.add(createClackAddMenu());
		...
		// if we right-clicked on a component, add a properties option
		if(cell_final != null) {
			...
			mi.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					if((cell_final instanceof ComponentCell)){		
						ClackComponent comp = ((ComponentCell)cell_final).getClackComponent();
						CompDoubleClickHandler.handleDoubleClick(graphpad.getCurrentDocument(), comp);
					}
				}
			});
		}
